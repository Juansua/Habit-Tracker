import { getLast7Days, getLast30Days, getDayLabel, getShortDate } from '../utils/dates'
import { computeCurrentStreak, computeBestStreak, computeTotalCompleted } from '../utils/streaks'

const StatBadge = ({ label, value, color }) => (
  <div className="flex flex-col items-center p-3 rounded-xl flex-1"
    ...
    <div className="flex gap-1.5 mt-3">
      {days.map((day) => {
        ...
        return (
          ...
          >
            <span className="text-[9px] text-slate-600 uppercase">{getDayLabel(day)}</span>
            <div
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 mb-1">
        <StatBadge label="Racha actual" value={streak === 0 ? '—' : `${streak}🔥`} color={habit.color} />
        <StatBadge label="Mejor racha" value={best === 0 ? '—' : `${best}⭐`} color="#f59e0b" />
        <StatBadge label="Total días" value={total} color="#94a3b8" />
      </div>

      {/* 7-day grid */}
      <MiniGrid7 habitId={habit.id} completions={completions} color={habit.color} onToggle={onToggle} />

      {/* 30-day heatmap */}
      <HeatMap30 habitId={habit.id} completions={completions} color={habit.color} onToggle={onToggle} />
    </div>
  )
}

export const StatsView = ({ habits, completions, onToggle }) => {
  if (habits.length === 0) {
    return (
      <div className="max-w-lg mx-auto px-4 text-center py-16">
        <div className="text-5xl mb-4">📊</div>
        <p className="text-slate-400 text-sm">Agrega hábitos para ver estadísticas</p>
      </div>
    )
  }
